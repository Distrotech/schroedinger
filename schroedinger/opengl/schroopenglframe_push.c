    ...
    void *texture_data, int y_offset, int height)
{
  ...
  texture_stride = dest->push_stride;
  texture_channels = SCHRO_FRAME_IS_PACKED (src->format) ? 1 : dest->channels;
...
schro_opengl_canvas_push (SchroOpenGLCanvas *dest, SchroFrameData *src)
{
  ...
  SCHRO_ASSERT (dest->type == SCHRO_OPENGL_CANVAS_TYPE_PRIMARAY);
  SCHRO_ASSERT (dest->format == src->format);
  SCHRO_ASSERT (dest->width == src->width);
  SCHRO_ASSERT (dest->height == src->height);
  ...
  if (SCHRO_OPENGL_CANVAS_IS_FLAG_SET (PUSH_RENDER_QUAD)) {
    ...
    glTexImage2D (GL_TEXTURE_RECTANGLE_ARB, 0, dest->internal_format,
        width, height, 0, dest->pixel_format, dest->storage_type, NULL);
  ...
  if ((SCHRO_OPENGL_CANVAS_IS_FLAG_SET (PUSH_U8_PIXELBUFFER) &&
    ...
    for (i = 0; i < SCHRO_OPENGL_PIXELBUFFER_BLOCKS; ++i) {
      pixelbuffer_height = dest->push_pixelbuffer->heights[i];

      glBindBufferARB (GL_PIXEL_UNPACK_BUFFER_EXT,
          dest->push_pixelbuffer->handles[i]);
      glBufferDataARB (GL_PIXEL_UNPACK_BUFFER_ARB,
          dest->push_stride * pixelbuffer_height, NULL, GL_STREAM_DRAW_ARB);
    ...
    for (i = 0; i < SCHRO_OPENGL_PIXELBUFFER_BLOCKS; ++i) {
      ...
      if (SCHRO_OPENGL_CANVAS_IS_FLAG_SET (PUSH_DRAWPIXELS)) {
        ...
        glDrawPixels (width, pixelbuffer_height, dest->pixel_format,
            dest->push_type, NULL);
    ...
    tmp_data = schro_opengl_get_tmp (dest->opengl, dest->push_stride * height);
    ...
    if (SCHRO_OPENGL_CANVAS_IS_FLAG_SET (PUSH_DRAWPIXELS)) {
      ...
      glDrawPixels (width, height, dest->pixel_format, dest->push_type,
         tmp_data);
